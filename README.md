...
<img src="team_LOGO.png" width="200"/>
...
[![D-Wave Neal](https://img.shields.io/badge/D--Wave_Neal-SA_Solver-blue)](https://docs.ocean.dwavesys.com/en/stable/docs_neal/sdk_index.html)
[![License](https://img.shields.io/badge/License-MIT-green)](LICENSE)

[📊 발표자료](QI4U%20in%20PNU%20-%20Group9.pdf) · [🚀 API 실행하기](#-quick-start) · [🖥️ Web UI](#-web-ui--api)
...
전통적인 농업에서는 균일한 격자 간격으로 작물을 심습니다. 하지만 실제 밭의 토양 비옥도는 위치마다 다릅니다.

Kyu-bit은 이 문제를 **양자 영감 최적화(Quantum-Inspired Optimization)** 로 해결합니다:

1. 밭을 격자로 나누고 각 위치에 토양 비옥도 점수를 부여
...
| | 기존 방식 | Kyu-bit |
|---|---|---|
| 배치 방법 | 균일 격자 배치 | 토양 상태 기반 **적응형 배치** |
| 의사결정 | 경험에 의존 | **수학적 최적화** (QUBO) |
| 작물 범위 | 단일 작물만 고려 | **12종 작물** 동시 비교 |
| 결과 확인 | 어려움 | **웹 UI + API + 히트맵** 제공 |
...
```
📥 입력          🧬 QUBO 생성       ⚡ SA 최적화        📤 결과 출력
─────────       ──────────────     ─────────────      ────────────
밭 크기(가로×세로)  12종 중 택1        목적함수+           JSON/Web
작물 선택         QUBO 행렬 생성     D-Wave Neal        히트맵 시각화
                                  Sampler
```
...
- **대각항**: `Q(i,i) = -(count_reward + fertility_weight × soil_score[i])` ← 많이 심되, 좋은 땅 우선
- **비대각항**: `Q(i,j) = +penalty (if distance(i,j) < min_spacing)` ← 너무 가까우면 페널티
...
| `GET` | `/health` | ❤️ 서버 상태 확인 |
| `GET` | `/api/crops` | 🌾 작물 목록 JSON |
| `POST` | `/api/optimize/single` | 🧬 단일 작물 최적화 |
| `POST` | `/api/optimize/all` | 📊 전체 작물 비교 |
...
개발자는 `/docs`에서 모든 API를 직접 테스트할 수 있습니다.
...
| Modeling | QUBO | 이진 비제약 이차 최적화 모델 |
...
| 파일 | 설명 |
|------|------|
| [QI4U in PNU - Group9.pdf](QI4U%20in%20PNU%20-%20Group9.pdf) | 📊 프로젝트 발표 자료 |

---

<div align="center">

**Made with ❤️ by Team Kyu-bit**
